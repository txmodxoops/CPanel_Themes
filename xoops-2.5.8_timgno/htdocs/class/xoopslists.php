 ...
 * @package     kernel
 * @since       2.0.0
 * @version     $Id: xoopslists.php 12666 2014-06-30 10:02:07Z beckmi $
 */
...
if (!defined('XOOPS_LISTS_INCLUDED')) {
    ...
    class XoopsLists
    {
        ...
        static function getAvatarsList($avatar_dir = '')
        {
            $avatars = array();
            if ($avatar_dir != '') {
                $avatars = XoopsLists::getImgListAsArray(XOOPS_UPLOAD_PATH . '/avatars/' . $avatar_dir . '/', $avatar_dir . '/');
            } else {
                $avatars = XoopsLists::getImgListAsArray(XOOPS_UPLOAD_PATH . '/avatars/');
            }
        ...
        static function getAllAvatarsList()
        {
            $avatars = array();
            $dirlist = array();
            $dirlist = XoopsLists::getDirListAsArray(XOOPS_UPLOAD_PATH . '/avatars/');
            if (count($dirlist) > 0) {
                foreach ($dirlist as $dir) {
                    $avatars[$dir] = &XoopsLists::getImgListAsArray(XOOPS_UPLOAD_PATH . '/avatars/' . $dir . '/', $dir . '/');
                }
            } else {
                return false;
            }
