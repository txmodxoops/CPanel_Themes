 ...
 * @version         $Id: formcheckbox.php 12537 2014-05-19 14:19:33Z beckmi $
 */
...
class XoopsFormCheckBox extends XoopsFormElement
{
    ...
    function render()
    {
        ...
        foreach ($ele_options as $value => $name) {
            ...
            $ret .= $ele_extra . ' />'."<label name='xolb_{$ele_name}' for='{$ele_id}{$id_ele}'><span></span>" . $name . "</label>" . $ele_delimeter ;
            if (! empty($this->columns)) {
                $ret .= '</td>';
                if (++ $i % $this->columns == 0) {
                    $ret .= '</tr>';
