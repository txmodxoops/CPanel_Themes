 ...
 * @subpackage      form
 * @since           2.0.0
 * @version         $Id: themeform.php 12537 2014-05-19 14:19:33Z beckmi $
 */
...
class XoopsThemeForm extends XoopsForm
{
    ...
    function render()
    {
        ...
        $ret = '<form class="form-horizontal" name="' . $ele_name . '" id="' . $ele_name . '" action="' . $this->getAction() . '" method="' . $this->getMethod() . '" onsubmit="return xoopsFormValidate_' . $ele_name . '();"' . $this->getExtra() . '>
        ...
        foreach ($this->getElements() as $ele) {
            if (!is_object($ele)) {
                ...
                if (!$ele->getNocolspan()) {
                    $ret .= '<tr valign="top" class="left"><td class="head">';
                    ...
                    $ret .= '<tr valign="top" class="left"><td class="head" colspan="2">';
                    if (($caption = $ele->getCaption()) != '') {
                        $ret .= '<div class="xoops-form-element-caption' . ($ele->isRequired() ? '-required' : '') . '">';
                        $ret .= '<span class="caption-text">' . $caption . '</span>';
                        $ret .= '<span class="caption-marker">*</span>';
                    ...
                    $ret .= '</td></tr><tr valign="top" class="left"><td class="' . $class . '" colspan="2">' . $ele->render() . '</td></tr>';
                }
            } else {
                $hidden .= $ele->render();
            }
