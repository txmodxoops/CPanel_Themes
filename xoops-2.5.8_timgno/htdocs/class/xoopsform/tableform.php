...
class XoopsTableForm extends XoopsForm
{
    ...
    function render()
    {
        $ret = $this->getTitle() . NWLINE
             . '<form class="form-horizontal" name="' . $this->getName() . '" id="' . $this->getName() . '" action="' . $this->getAction() . '" method="' . $this->getMethod() . '"' . $this->getExtra() . '>'
             . NWLINE . '<table class="table table-bordered">' . NWLINE;
        $hidden = "";
        foreach ($this->getElements() as $ele) {
            if (!$ele->isHidden()) {
                if (!$ele->getNocolspan()) {
                    $ret .= '<tr valign="top" class="left"><td>' . $ele->getCaption();
                    if ($ele_desc = $ele->getDescription()) {
                        $ret .= '<br /><br /><span style="font-weight: normal;">' . $ele_desc . '</span>';
                    }
                    $ret .= '</td><td>' . $ele->render() . '</td></tr>';
                } else {
                    $ret .= '<tr valign="top" class="left"><td colspan="2">' . $ele->getCaption();
                    $ret .= '</td></tr><tr valign="top" class="left"><td>' . $ele->render() . '</td></tr>';
                }
            } else {
