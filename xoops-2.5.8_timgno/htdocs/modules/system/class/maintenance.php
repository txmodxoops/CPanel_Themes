 ...
 * @package     system
 * @version     $Id: maintenance.php 12360 2014-03-08 09:46:59Z beckmi $
 */
...
class SystemMaintenance
{
    ...
    {
        $ret = '<table class="table table-bordered"><thead><tr class="text-center"><th>'._AM_SYSTEM_MAINTENANCE_TABLES1.'</th><th>'._AM_SYSTEM_MAINTENANCE_TABLES_OPTIMIZE.'</th><th>'._AM_SYSTEM_MAINTENANCE_TABLES_CHECK.'</th><th>'._AM_SYSTEM_MAINTENANCE_TABLES_REPAIR.'</th><th>'._AM_SYSTEM_MAINTENANCE_TABLES_ANALYZE.'</th></tr></thead><tbody>';
        $tab = array();
        for ($i=0; $i<4; $i++) {
            $tab[$i] = $i+1;
        }
        ...
        for ($i=0; $i<count($tables); $i++) {
            $ret .= '<tr class="'.$class.'"><td class="text-center">'.$this->prefix.$tables[$i].'</td>';
            for ($j=0; $j<4; $j++) {
                if ($tab1[$j] == 1) {
                    // Optimize
                    $result = $this->db->queryF('OPTIMIZE TABLE '.$this->prefix.$tables[$i]);
                    if ($result) {
                        $ret .= '<td class="xo-actions text-center"><img src="'.system_AdminIcons('success.png').'" /></td>';
                    } else {
                        $ret .= '<td class="xo-actions text-center"><img src="'.system_AdminIcons('cancel.png').'" /></td>';
                    }
                } elseif ($tab1[$j] == 2) {
                    // Check tables
    ...
    function dump_tables($tables, $drop)
    {
        ...
        $ret[1] = '<table class="table table-bordered"><thead><tr class="text-center"><th width="30%">'._AM_SYSTEM_MAINTENANCE_DUMP_TABLES.'</th><th width="35%">'._AM_SYSTEM_MAINTENANCE_DUMP_STRUCTURES.'</th><th  width="35%">'._AM_SYSTEM_MAINTENANCE_DUMP_NB_RECORDS.'</th></tr></thead><tbody>';
        $class = 'odd';
        for ($i=0; $i<count($tables); $i++) {
            //structure
    ...
    function dump_modules($modules, $drop)
    {
        ...
        for ($i=0; $i<count($modules); $i++) {
            ...
            if ($modtables != false && is_array($modtables)) {
                ...
                $ret[1] .= '<tr><td colspan="3" class="text-center">'._AM_SYSTEM_MAINTENANCE_DUMP_NO_TABLES.'</td></tr>';
            }
        }
        $ret[1] .= '</tbody></table>';
        $ret = $this->dump_write($ret);
    ...
    function dump_table_structure($ret, $table, $drop, $class)
    {
        ...
        $ret[1] .= '<tr class="'.$class.'"><td class="text-center">'.$table.'</td><td class="xo-actions text-center">';
    ...
    function dump_write($ret)
    {
        ...
        if ( file_put_contents($path_file, $ret[0]) ) {
            $ret[1] .= '<table class="table table-bordered"><thead><tr class="text-center"><th colspan="2">'._AM_SYSTEM_MAINTENANCE_DUMP_FILE_CREATED.'</th><th>'._AM_SYSTEM_MAINTENANCE_DUMP_RESULT.'</th></tr></thead><tbody><tr class="text-center"><td colspan="2"><a href="'.XOOPS_URL.'/modules/system/admin/maintenance/dump/'.$file_name.'">'.$file_name.'</a></td><td class="xo-actions"><img src="'.system_AdminIcons('success.png').'" /></td><tr></tbody></table>';
        } else {
            $ret[1] .= '<table class="table table-bordered"><thead><tr class="text-center"><th colspan="2">'._AM_SYSTEM_MAINTENANCE_DUMP_FILE_CREATED.'</th><th>'._AM_SYSTEM_MAINTENANCE_DUMP_RESULT.'</th></tr></thead><tbody><tr class="text-center"><td colspan="2" class="xo-actions">'.$file_name.'</td><td class="xo-actions"><img src="'.system_AdminIcons('cancel.png').'" /></td><tr></tbody></table>';
        }
        return $ret;
    }
}
