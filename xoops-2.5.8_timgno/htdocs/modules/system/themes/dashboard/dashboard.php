 ...
 * @version     1.2
 * @version     $Id: dashboard.php 12852 2014-12-27 09:14:37Z timgno $
 */

/**
 * Class XoopsGuiDashboard
 */
class XoopsGuiDashboard extends XoopsSystemGui
{
    ...
    function header()
    {
        ...
        $xoTheme->addScript(XOOPS_ADMINTHEME_URL . '/dashboard/assets/js/bootstrap.min.js');
		$xoTheme->addScript(XOOPS_ADMINTHEME_URL . '/dashboard/assets/js/styleswitch.js');
        $xoTheme->addScript(XOOPS_ADMINTHEME_URL . '/dashboard/assets/js/formenu.js');
        $xoTheme->addScript(XOOPS_ADMINTHEME_URL . '/dashboard/assets/js/menu.js');
        $xoTheme->addScript(XOOPS_ADMINTHEME_URL . '/dashboard/assets/js/tooltip.js');
        $xoTheme->addScript(XOOPS_ADMINTHEME_URL . '/dashboard/assets/js/tabs.jquery.tools.min.js');*/
        
        /*$xoTheme->addStylesheet(XOOPS_ADMINTHEME_URL . '/dashboard/assets/css/style.css' );
        $xoTheme->addStylesheet(XOOPS_ADMINTHEME_URL . '/dashboard/assets/css/dark.css', array('title' => 'dark', 'media' => 'screen'));
        $xoTheme->addStylesheet(XOOPS_ADMINTHEME_URL . '/dashboard/assets/css/silver.css', array('title' => 'silver', 'media' => 'screen'));
        $xoTheme->addStylesheet(XOOPS_ADMINTHEME_URL . '/dashboard/assets/css/orange.css', array('title' => 'orange', 'media' => 'screen'));*/
        ...
        $menu[1]['absolute'] = 1;
        $menu[1]['icon'] = XOOPS_ADMINTHEME_URL . '/dashboard/assets/img/xoops.png';
        $menu[2]['link'] = XOOPS_URL . '/user.php?op=logout';
        $menu[2]['title'] = _LOGOUT;
        $menu[2]['absolute'] = 1;
        $menu[2]['icon'] = XOOPS_ADMINTHEME_URL . '/dashboard/assets/img/logout.png';
        ...
        if (empty($xoopsModule) || 'system' == $xoopsModule->getVar('dirname', 'n')) {
            ...
            foreach (array_keys($mod_options) as $item) {
                ...
                $mod_options[$item]['icon'] = empty($mod_options[$item]['icon']) ? '' : XOOPS_ADMINTHEME_URL . '/dashboard/assets/' . $mod_options[$item]['icon'];
        ...
        $tpl->append('navitems', array('link' => XOOPS_URL . '/modules/system/admin.php?fct=modulesadmin', 'title' => _AM_SYSTEM_MODULES, 'icon' => 'dropbox', 'dir' => $mod->getVar('dirname', 'n'), 'menu' => $menu));
        ...
        $OPT[] = array(
            ...
            'absolute'  => 1,
            'icon'     => XOOPS_ADMINTHEME_URL . '/dashboard/assets/icons/prefs_small.png');
        $OPT[] = array(
        ...
        $menu[] = array(
            ...
            'absolute'  => 1,
            'icon'     => XOOPS_ADMINTHEME_URL . '/dashboard/assets/img/xoops.png');
        $menu[] = array(
        ...
        $menu[] = array(
            ...
            'absolute'  => 1,
            'icon'     => XOOPS_ADMINTHEME_URL . '/dashboard/assets/img/tweb.png');

        $tpl->append('navitems', array('link' => XOOPS_URL . '/admin.php', 'title' => _OXYGEN_INTERESTSITES, 'icon' => 'sitemap', 'menu' => $menu));

        //add OPTIONS/links for local support
        if (file_exists($file = XOOPS_ADMINTHEME_PATH . '/dashboard/language/' . $xoopsConfig['language'] . '/localsupport.php' )) {
            $links = include XOOPS_ADMINTHEME_PATH . '/dashboard/language/' . $xoopsConfig['language'] . '/localsupport.php';
            if ( count($links) > 0 ) {
		...
		$tpl->assign ( 'avatar', $xoopsUser->user_avatar() );
		$tpl->assign ( 'admin_avatar', XOOPS_UPLOAD_URL . "/" . $xoopsUser->user_avatar() );
    }
