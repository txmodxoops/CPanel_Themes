 ...
 * @version         $Id: header.php 12666 2014-06-30 10:02:07Z beckmi $
 */
...
if ($xoopsConfig['theme_set'] != 'default' && file_exists(XOOPS_THEME_PATH . '/' . $xoopsConfig['theme_set'] . '/theme.php')) {
	...
	/*if (@is_object($xoTheme->plugins['xos_logos_PageBuilder'])) {
        ...
        $xoopsTpl->assign_by_ref('xoops_lblocks', $aggreg->blocks['canvas_left']);
        $xoopsTpl->assign_by_ref('xoops_rblocks', $aggreg->blocks['canvas_right']);
        $xoopsTpl->assign_by_ref('xoops_ctcblocks', $aggreg->blocks['page_topcenter']);
        $xoopsTpl->assign_by_ref('xoops_ctlblocks', $aggreg->blocks['page_topleft']);
        $xoopsTpl->assign_by_ref('xoops_ctrblocks', $aggreg->blocks['page_topright']);
		$xoopsTpl->assign_by_ref('xoops_cbcblocks', $aggreg->blocks['page_bottomcenter']);
        $xoopsTpl->assign_by_ref('xoops_cblblocks', $aggreg->blocks['page_bottomleft']);
        $xoopsTpl->assign_by_ref('xoops_cbrblocks', $aggreg->blocks['page_bottomright']);
		$xoopsTpl->assign_by_ref('xoops_fcblocks', $aggreg->blocks['footer_center']);
        $xoopsTpl->assign_by_ref('xoops_flblocks', $aggreg->blocks['footer_left']);
        $xoopsTpl->assign_by_ref('xoops_frblocks', $aggreg->blocks['footer_right']);
        $xoopsTpl->assign('xoops_showlblock', !empty($aggreg->blocks['canvas_left']));
        $xoopsTpl->assign('xoops_showrblock', !empty($aggreg->blocks['canvas_right']));
        $xoopsTpl->assign('xoops_showctblock', !empty($aggreg->blocks['page_topcenter']) || !empty($aggreg->blocks['page_topleft']) || !empty($aggreg->blocks['page_topright']));
		$xoopsTpl->assign('xoops_showcbblock', !empty($aggreg->blocks['page_bottomcenter']) || !empty($aggreg->blocks['page_bottomleft']) || !empty($aggreg->blocks['page_bottomright']));
		$xoopsTpl->assign('xoops_showfblock', !empty($aggreg->blocks['footer_center']) || !empty($aggreg->blocks['footer_left']) || !empty($aggreg->blocks['footer_right']));
    }*/
